...
while (janela.isOpen()){
    ...
    //****************************************************************************
        ...
        for(j=0;j<4;j++){
        matrizproxPeca[coordenadasdaspecas[aleatorioPeca][j].py][coordenadasdaspecas[aleatorioPeca][j].px]=true;//mov_y e mov_x usados para mover a peca
        }
        ...
        for(i=0;i<4;i++){
            for(j=0;j<4;j++){
                if(matrizproxPeca[i][j]){
                  ...
                  texturapecaEstatica.loadFromFile(cores[corDaMatridecor]);
