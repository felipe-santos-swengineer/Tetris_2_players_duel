...
int TelaJogo(){
   ...
   nomJog2.setPosition(709,68);
    ...
    int tempoPerdeu1Min, tempoPerdeu1Seg;//qdo o jogador morre, faz-se o backup do tempo nessas variaveis
    int tempoPerdeu2Min, tempoPerdeu2Seg;
...
*/
    ...
    FILE *fileRanking1=std::fopen("Ranking_1_Lugar.txt","r");
    std::fscanf(fileRanking1,"%d ", &pontuacao1lugar);
    std::fclose(fileRanking1);

    int pontuacao2lugar=0;
    FILE *fileRanking2=std::fopen("Ranking_2_Lugar.txt","r");
    std::fscanf(fileRanking2,"%d ", &pontuacao2lugar);
    std::fclose(fileRanking2);

    int pontuacao3lugar=0;
    FILE *fileRanking3=std::fopen("Ranking_3_Lugar.txt","r");
    std::fscanf(fileRanking3,"%d ", &pontuacao3lugar);
    std::fclose(fileRanking3);
    ...
    while(i>0){
        ...
        for(i=0;i<10;i++){
            if(matrizPlayerEstatico[i][0]==true){
                ...
                jogador1perdeu=true;
                tempoPerdeu1Min=tempominutos;
                tempoPerdeu1Seg=temposegundos;
            }
            if(matrizPlayerEstatico_2[i][0]==true){
                janela.draw(spritePerdeu2);
                jogador2perdeu=true;
                tempoPerdeu2Min=tempominutos;
                tempoPerdeu2Seg=temposegundos;
            }
            if(jogador1perdeu==true&&jogador2perdeu==true){//vai ser setados os recordes, se houver
                int auxInt1, auxInt2, auxInt3;
                char auxChar1[10];
                int auxInt4, auxInt5, auxInt6;
                char auxChar2[10];
                int auxInt7, auxInt8, auxInt9;
                char auxChar3[10];
                 FILE *filePegarArqlugar=std::fopen("Ranking_1_Lugar.txt", "r");
                std::fscanf(filePegarArqlugar,"%d%s%d%d\n", &auxInt1, auxChar1, auxInt2, auxInt3);
                std::fclose(filePegarArqlugar);
                     filePegarArqlugar=std::fopen("Ranking_2_Lugar.txt", "r");
                std::fscanf(filePegarArqlugar,"%d%s%d%d\n", &auxInt4, auxChar2, auxInt5, auxInt6);
                std::fclose(filePegarArqlugar);
                     filePegarArqlugar=std::fopen("Ranking_3_Lugar.txt", "r");
                std::fscanf(filePegarArqlugar,"%d%s%d%d\n", &auxInt7, auxChar3, auxInt8, auxInt9);
                std::fclose(filePegarArqlugar);

                if(pontos>pontuacao1lugar){
                    std::rename("Ranking_1_Lugar.txt", "Ranking_1_Lugar(alt).txt");//pra nao gerar conflito na renomeaçao com nomes repetidos
                    ...
                    std::fprintf(fileAlteraVencedor,"%s %d\n", n1, pontos);
                    ...
                    CodTela=6;//tela recorde
                    return 0;
                } else if(pontos==pontos_2&&pontos_2>pontuacao1lugar){//qundo ha um recorde e empate a mesmo tempo, quem tem o maior tempo vence
                    std::rename("Ranking_1_Lugar.txt", "Ranking_1_Lugar(alt).txt");//pra nao gerar conflito na renomeaçao com nomes repetidos
                    std::rename("Ranking_2_Lugar.txt", "Ranking_2_Lugar(alt).txt");
                    std::rename("Ranking_3_Lugar.txt", "Ranking_3_Lugar(alt).txt");
                    ...
                    if(tempoPerdeu1Min>tempoPerdeu2Min){

                        std::fprintf(fileAlteraRanking,"%d %s %d %d\n", pontos, n1,tempoPerdeu1Min, tempoPerdeu1Seg);
                        std::fclose(fileAlteraRanking);
                        FILE *fileAlteraVencedor=std::fopen("VencedorPartidaAtual.txt", "w");
                        std::fprintf(fileAlteraVencedor,"%s %d\n", n1, pontos);
                        std::fclose(fileAlteraVencedor);
                        CodTela=6;//tela recorde
                        return 0;
                    } else if(tempoPerdeu1Min<tempoPerdeu2Min){

                        std::fprintf(fileAlteraRanking,"%d %s %d %d\n", pontos_2, n2,tempoPerdeu2Min, tempoPerdeu2Seg);
                        std::fclose(fileAlteraRanking);
                        FILE *fileAlteraVencedor=std::fopen("VencedorPartidaAtual.txt", "w");
                        std::fprintf(fileAlteraVencedor,"%s %d\n", n2, pontos_2);
                        std::fclose(fileAlteraVencedor);
                        CodTela=6;//tela recorde
                        return 0;
                    } else if(tempoPerdeu1Min==tempoPerdeu2Min){// se empatar nos minutos, sera desempatado nos segundos
                        if(tempoPerdeu1Seg>tempoPerdeu2Seg){
                            std::fprintf(fileAlteraRanking,"%d %s %d %d\n", pontos, n1,tempoPerdeu1Min, tempoPerdeu1Seg);
                            std::fclose(fileAlteraRanking);
                            FILE *fileAlteraVencedor=std::fopen("VencedorPartidaAtual.txt", "w");
                            std::fprintf(fileAlteraVencedor,"%s %d\n", n1, pontos);
                            ...
                            CodTela=6;//tela recorde
                            return 0;
                        } else if(tempoPerdeu1Seg<tempoPerdeu2Seg){
                            std::fprintf(fileAlteraRanking,"%d %s %d %d\n", pontos_2, n2,tempoPerdeu2Min, tempoPerdeu2Seg);
                            std::fclose(fileAlteraRanking);
                            FILE *fileAlteraVencedor=std::fopen("VencedorPartidaAtual.txt", "w");
                            std::fprintf(fileAlteraVencedor,"%s %d\n", n2, pontos_2);
                ...
                } else {//nao teve recorde, mas pode ter novos 2 e 3 lugar
                    if(pontos>pontos_2&&pontos>auxInt7){//jogador 1 ganhou e e maior q o 3 lugar
                        if(pontos<auxInt4){//se for menor q 2 lugar
                            FILE *fileOutrosColocados=std::fopen("Ranking_3_Lugar.txt","w");
                            std::fprintf(fileOutrosColocados,"%d %s %d %d\n", pontos, nomJog1, tempoPerdeu1Min, tempoPerdeu1Seg);
                            std::fclose(fileOutrosColocados);

                        } else if(pontos>auxInt4){//ele e maior que o 2 lugar, vai fica em segundo, e o segundo subst o terceiro
                            std::rename("Ranking_2_Lugar.txt", "Ranking_2_Lugar(alt).txt");
                            std::rename("Ranking_3_Lugar.txt", "Ranking_3_Lugar(alt).txt");
                            std::rename("Ranking_2_Lugar(alt).txt", "Ranking_3_Lugar.txt");

                            FILE *fileOutrosColocados=std::fopen("Ranking_2_Lugar.txt", "w");
                            std::fprintf(fileOutrosColocados,"%d %s %d %d\n", pontos, nomJog1, tempoPerdeu1Min, tempoPerdeu1Seg);
                            std::fclose(fileOutrosColocados);
                        }
                        FILE *fileAlteraVencedor=std::fopen("VencedorPartidaAtual.txt", "w");
                        ...
                        CodTela=7;
                        return 0;// TelaVencedor();
                    } else if(pontos_2>pontos&&pontos_2>auxInt7){
                        if(pontos_2<auxInt4){//se for menor q 2 lugar
                            FILE *fileOutrosColocados=std::fopen("Ranking_3_Lugar.txt","w");
                            std::fprintf(fileOutrosColocados,"%d %s %d %d\n", pontos_2, nomJog2, tempoPerdeu2Min, tempoPerdeu2Seg);
                            std::fclose(fileOutrosColocados);

                        } else if(pontos_2>auxInt4){//ele e maior que o 2 lugar, vai fica em segundo, e o segundo subst o terceiro
                            std::rename("Ranking_2_Lugar.txt", "Ranking_2_Lugar(alt).txt");
                            std::rename("Ranking_3_Lugar.txt", "Ranking_3_Lugar(alt).txt");
                            std::rename("Ranking_2_Lugar(alt).txt", "Ranking_3_Lugar.txt");
                    ...
                    if(pontos>pontos_2){
                        //std::fprintf(fileAlteraVencedor,"12345 marco\n");
                        std::fprintf(fileAlteraVencedor,"%s %d\n", n1, pontos);
                        std::fclose(fileAlteraVencedor);
                        CodTela=7;
                        return 0;// TelaVencedor();
                    } else if(pontos<pontos_2){
                        //std::fprintf(fileAlteraVencedor,"12345 aurelio\n");
                        std::fprintf(fileAlteraVencedor,"%s %d\n", n2, pontos_2);
                        std::fclose(fileAlteraVencedor);
                        CodTela=7;
                        return 0;// TelaVencedor();
