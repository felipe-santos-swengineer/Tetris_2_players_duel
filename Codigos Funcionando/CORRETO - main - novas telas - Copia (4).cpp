...
*/
    ...
    FILE *fileRanking=std::fopen("Ranking_1_Lugar.txt","r");//esta pegando a pontuaçao do 1 lugar como base para saber se tem recorde
...
*/
   ...
   temposegundos = (int) clockCronometro.getElapsedTime().asSeconds(); //esta sendo feito o casting de float para inteiro
    ...
    while(i>0){
        ...
        for(i=0;i<10;i++){
            if(!jogador1perdeu){//condicional para ele so entrar uma vez, pois tem algoritmo de gravaçao em arquivo

                if(matrizPlayerEstatico[i][0]==true){
                    if(jogador1perdeu==false&&jogador2perdeu==false){
                    FILE *fileResultado=std::fopen("ResultadoDoJogo.txt", "w");
                    std::fclose(fileResultado);
                    }

                    FILE *fileResultado1=std::fopen("ResultadoDoJogo.txt", "a");
            ...
            if(!jogador2perdeu){
                ...
                if(matrizPlayerEstatico_2[i][0]==true){
                    ...
                    FILE *fileResultado2=std::fopen("ResultadoDoJogo.txt", "a");
...
int quemGanhou(){
    ...
    FILE *file=std::fopen("ResultadoDoJogo.txt", "r");
    ...
    fclose(file);
        ...
        file=std::fopen("Ranking_2_Lugar.txt", "r");
    std::fscanf(file,"%d %s %d %d\n",&pontos2lugar, Nome2lugar, &min2lugar, &seg2lugar);
    fclose(file);

        file=std::fopen("Ranking_3_Lugar.txt", "r");
    ...
    FILE *fileVencedor=std::fopen("VencedorPartidaAtual.txt", "w");

    if(pontos_1>pontos1lugar||pontos_2>pontos1lugar){//se alguem bateu recorde
        printf("ALGUEM BATEU RECORDE\n");
        std::rename("Ranking_1_Lugar.txt", "Ranking_1_Lugar(alt).txt");
        std::rename("Ranking_2_Lugar.txt", "Ranking_2_Lugar(alt).txt");
        std::rename("Ranking_3_Lugar.txt", "Ranking_3_Lugar(alt).txt");

        std::rename("Ranking_1_Lugar(alt).txt", "Ranking_2_Lugar.txt");
        std::rename("Ranking_2_Lugar(alt).txt", "Ranking_3_Lugar.txt");



        if(pontos_1>pontos1lugar){//recorde jog 1
            printf("JOGADO 1 FEZ RECORDE\n");
            std::rename("Ranking_3_Lugar(alt).txt", "Ranking_1_Lugar.txt");
            file=std::fopen("Ranking_1_Lugar.txt", "w");
            std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
            std::fclose(file);

            std::fprintf(fileVencedor, "%s %d", nomeJog1, pontos_1);
            std::fclose(fileVencedor);

        } else if(pontos_2>pontos1lugar){//recorde jog 2
            printf("JOGADO 2 FEZ RECORDE\n");
            std::rename("Ranking_3_Lugar(alt).txt", "Ranking_1_Lugar.txt");
            file=std::fopen("Ranking_1_Lugar.txt", "w");
            std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
            std::fclose(file);

            std::fprintf(fileVencedor, "%s %d", nomeJog2, pontos_2);
            std::fclose(fileVencedor);
        } else if(pontos_1==pontos_2&&pontos_2==pontos1lugar){//os dois atingem recorde, desempare pelo tempo
            printf("JOGADO OS DOIS TIRARAM A MESMA PONTUAÇAO\n");
            if(min_1>min_2){//compara os minutos, se for maior ja vence logo. se for igual, compara os segundos
                printf("JOGADO 1 TEVE MAIOR MINUTOS\n");
                std::rename("Ranking_3_Lugar(alt).txt", "Ranking_1_Lugar.txt");
                file=std::fopen("Ranking_1_Lugar.txt", "w");
                std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
                std::fclose(file);
                ...
                printf("JOGADO 2 TEVE MAIOR MINUTOS\n");
                std::rename("Ranking_3_Lugar(alt).txt", "Ranking_1_Lugar.txt");
                file=std::fopen("Ranking_1_Lugar.txt", "w");
                std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
                std::fclose(file);
                ...
                std::fprintf(fileVencedor, "%s %d", nomeJog2, pontos_2);
                std::fclose(fileVencedor);
            } else if(min_2==min_1){//compara os segundos
                printf("MESMA QUANTIDADE DE MINUTOS\n");
                if(seg_1>seg_2){
                    printf("JOGADO 1 TEVE MAIOR SEGUNDOS\n");
                    std::rename("Ranking_3_Lugar(alt).txt", "Ranking_1_Lugar.txt");
                    file=std::fopen("Ranking_1_Lugar.txt", "w");
                    std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
                    std::fclose(file);
                    ...
                    printf("JOGADO 2 TEVE MAIOR SEGUNDOS\n");
                    std::rename("Ranking_3_Lugar(alt).txt", "Ranking_1_Lugar.txt");
                    file=std::fopen("Ranking_1_Lugar.txt", "w");
                    std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
                    std::fclose(file);
    ...
    } else {//nao houve recorde
        printf("NAO HOUVE RECORDE\n");
        if(pontos_1>pontos_2){//se nao houver empate, haveraa diferença de pontos
            printf("JOGADO 1 TEVE MAIOR PONTOS\n");
            std::rename("Ranking_2_Lugar.txt", "Ranking_2_Lugar(alt).txt");
            std::rename("Ranking_3_Lugar.txt", "Ranking_3_Lugar(alt).txt");
            //jogador 1
            if(pontos_1>pontos3lugar&&pontos_1<pontos2lugar){//maior q 3 lugar, menor q segundo. vai ficar  em 3 lugar
                printf("PONTOS JOG1 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 1 MENOR QUE 2 LUGAR\n");
                std::rename("Ranking_3_Lugar(alt).txt", "Ranking_3_Lugar.txt");
                file=std::fopen("Ranking_3_Lugar.txt", "w");
                std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
                std::fclose(file);

                std::fprintf(fileVencedor, "%s %d", nomeJog1, pontos_1);
                std::fclose(fileVencedor);
            } else if(pontos_1>pontos3lugar&&pontos_1>pontos2lugar){//maior q 3 lugar, maior q 2 lugar. vai ficar em 2;
                printf("PONTOS JOG1 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 1 MAIOR QUE 2 LUGAR\n");
                std::rename("Ranking_2_Lugar(alt).txt", "Ranking_3_Lugar.txt");
                std::rename("Ranking_3_Lugar(alt).txt", "Ranking_2_Lugar.txt");
                file=std::fopen("Ranking_2_Lugar.txt", "w");
                std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
                std::fclose(file);
            ...
            //jogador 2
            if(pontos_2>pontos3lugar&&pontos_2<pontos3lugar){//maior q 3 lugar, menor q segundo. vai ficar  em 3 lugar
                printf("PONTOS JOG2 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 2 MENOR QUE 3 LUGAR\n");
                std::rename("Ranking_3_Lugar(alt).txt", "Ranking_3_Lugar.txt");
                file=std::fopen("Ranking_3_Lugar.txt", "w");
                std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
                std::fclose(file);
                ...
                std::fprintf(fileVencedor, "%s %d", nomeJog2, pontos_2);
                std::fclose(fileVencedor);
            } else if(pontos_2>pontos3lugar&&pontos_2>pontos2lugar){//maior q 3 lugar, maior q 2 lugar. vai ficar em 2;
                printf("PONTOS JOG2 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 2 MAOIR QUE 2 LUGAR\n");
                std::rename("Ranking_2_Lugar(alt).txt", "Ranking_3_Lugar.txt");
                std::rename("Ranking_3_Lugar(alt).txt", "Ranking_2_Lugar.txt");
                file=std::fopen("Ranking_2_Lugar.txt", "w");
                std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
            ...
            printf("SEM RECORDE, EMPATE DE PONTOS\n");
            if(min_1>min_2){
                printf("MIN JOG1 MAIOR QUE MIN JOG 2\n");
                    if(pontos_1>pontos3lugar&&pontos_1<pontos2lugar){//vai pra 3 lugar
                        printf("PONTOS JOG1 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 1 MENOR QUE 2 LUGAR\n");
                        file=std::fopen("Ranking_3_Lugar.txt", "w");
                        std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
                        std::fclose(file);
                ...
                printf("MIN JOG 2 MAIOR MIN JOG 1\n");
                    if(pontos_2>pontos3lugar&&pontos_2<pontos2lugar){//vai pra 3 lugar
                        printf("PONTOS JOG2 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 2 MENOR QUE 2 LUGAR\n");
                        file=std::fopen("Ranking_3_Lugar.txt", "w");
                        std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
                        std::fclose(file);
            ...
            } else if(min_1==min_2){//minutos iguais, desempata pelos segundos
                printf("MINUTOS JOG1 IGUAL MIN JOG2\n");
                if(seg_1>seg_2){
                    printf("SEG JOG1 MAIOR SEG JOG2\n");
                    if(pontos_1>pontos3lugar&&pontos_1<pontos2lugar){//vai pra 3 lugar
                        printf("PONTOS JOG1 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 1 MENOR QUE 2 LUGAR\n");
                        file=std::fopen("Ranking_3_Lugar.txt", "w");
                        std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
                    ...
                    if(pontos_2>pontos3lugar&&pontos_2<pontos2lugar){//vai pra 3 lugar
                        ...
                        printf("PONTOS JOG2 MAIOR QUE PONTOS 3 LUGAR E PONTOS JOG 2 MAIOR QUE 2 LUGAR\n");
                        std::rename("Ranking_2_Lugar(alt).txt", "Ranking_3_Lugar.txt");
                        std::rename("Ranking_3_Lugar(alt).txt", "Ranking_2_Lugar.txt");
                        file=std::fopen("Ranking_2_Lugar.txt", "w");
                        std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
    ...
    if(empate){
            printf("empateeeeeeeeeeeeeeeeeeeeeeeeeeeeeee");
...
int main(){
    ...
    std::fclose(file);
    //file=std::fopen("VencedorPartidaAtual.txt", "w");
    //std::fclose(file);
